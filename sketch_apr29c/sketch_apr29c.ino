...
void setup() {
  ...
  String encStr = "i love dogs" ;
  int len = encStr.length()+1;
  encStr.toCharArray(cstr, len);

  for(i=0;i<len;i++)
    data[i] = (uint8_t)cstr[i];

  DUMP("Unencrypted data: ", i, data, sizeof(data));
  Serial.println("encStr = " + encStr);
  
  aes256_encrypt_ecb(&ctxt, data);
  DUMP("Encrypted data: ", i, data, sizeof(data));
  Serial.println("encrypted string = " + String();
